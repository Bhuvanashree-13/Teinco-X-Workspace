...
router.get('/:currency', async (req, res) => {
  ...
  if (currency === 'INR') return res.json({ base: 'INR', quote: 'INR', rate: 1, date: new Date().toISOString().slice(0, 10), source: 'fixed' })
  ...
  try {
    ...
    cache.set(currency, { rate, date, expiresAt: Date.now() + CACHE_TTL_MS })
