...
export default function AskAI() {
  ...
  async function submit(event: FormEvent) {
    ...
    const prompt = question.trim(), history = messages.slice(-12).map(({ role, content }) => ({ role, content }))
    setBusy(true); setError(''); setAnswer(null); setPreview(false); setMessages(current => [...current, { role: 'user', content: prompt }]); setQuestion('')
    try { const result: Evidence = await apiPost('/flow/ask', { question: prompt, period, history }); setAnswer(result); setMessages(current => [...current, { role: 'assistant', content: result.message || 'Here is what I found.', evidence: result }]) }
