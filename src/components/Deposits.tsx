...
export default function Deposits() {
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      depositDate: new Date(item.depositDate).toISOString().slice(0, 10),
      source: item.source || '',
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) await apiPut(`/deposits/${editingId}`, payload)
      else await apiPost('/deposits', payload)
      setForm(defaults); setEditingId(null); setShowForm(false); setMessage(editingId ? 'Deposit updated successfully.' : 'Deposit recorded successfully.'); await refetch()
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && <div className="mobile-dialog-overlay" onMouseDown={() => setShowForm(false)}><div className="mobile-dialog-panel mobile-dialog-panel-lg" onMouseDown={e => e.stopPropagation()}><div className="mobile-dialog-header"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30">{editingId ? <Pencil className="h-5 w-5" /> : <ArrowDownToLine className="h-5 w-5" />}</div><div><h3 className="text-lg font-semibold dark:text-white">{editingId ? 'Edit deposit' : 'Record deposit'}</h3><p className="text-sm text-gray-500">Foreign currency is automatically converted to INR.</p></div></div><button type="button" onClick={() => setShowForm(false)} className="mobile-dialog-close"><X className="h-5 w-5" /></button></div>
        <form onSubmit={submit} className="mobile-dialog-form">
          ...
          <div className="mobile-form-actions dark:border-gray-700"><button type="button" onClick={() => setShowForm(false)} className="rounded-lg border px-4 py-2 text-sm">Cancel</button><button disabled={saving || inrAmount <= 0} className="brand-primary-button">{saving ? 'Saving…' : editingId ? 'Update deposit' : 'Save deposit'}</button></div>
        </form></div></div>}
    </div>
  )
}
