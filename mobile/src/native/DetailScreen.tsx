...
export function DetailScreen({ route, navigation }: NativeStackScreenProps<RootStack, 'Detail'>) {
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <View style={{ alignItems: 'center', paddingVertical: 15, gap: 10, paddingHorizontal: 20 }}>{module.id === 'projects' || module.id === 'taskboard' ? <ProjectLogo logoDataUrl={module.id === 'projects' ? row.logoDataUrl : row.project?.logoDataUrl} color={module.id === 'projects' ? row.color : row.project?.color} size={84} /> : <View style={{ backgroundColor: colors.softBlue, padding: 20, borderRadius: 24 }}><Icon name={module.icon} size={32} /></View>}<Text style={[s.title, { textAlign: 'center', fontSize: 23 }]} numberOfLines={2}>{recordTitle(module.id, row)}</Text>{amount !== null && <Text style={[s.title, { fontSize: 34 }]}>{currency(amount, module.id === 'subscriptions' ? row.currency : 'INR')}</Text>}<Tag value={row.status || row.priority} /><Text style={s.caption}>{recordDate(row) ? shortDate(recordDate(row)) : ''}</Text></View>
    ...
    <Panel>
      ...
      {module.id === 'balances' && <><RowValue label="Leave cycle" value={row.cycleLabel} /><RowValue label="Cycle dates" value={row.cycleStart && row.cycleEnd ? `${shortDate(row.cycleStart)} – ${shortDate(row.cycleEnd)}` : null} /><RowValue label="Monthly allowance" value={`${row.monthlyAllowance || 2} days`} /><RowValue label="Accrued allowance" value={`${row.allowance} days`} /><RowValue label="Used" value={`${row.used} days`} /><RowValue label="Available" value={`${row.balance} days`} /></>}
      {module.id === 'vendors' && role === 'admin' && <><RowValue label="This year spending" value={currency(row.currentYearSpend)} /><RowValue label="Last year spending" value={currency(row.previousYearSpend)} /></>}
