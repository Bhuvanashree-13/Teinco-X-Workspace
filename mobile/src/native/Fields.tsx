...
export function NativeField({ field, value, onChange, disabled, existingLabel }: { field: Field; value: any; onChange: (value: any) => void; disabled?: boolean; existingLabel?: string }) {
  ...
  return <View style={styles.field}>
    <View style={styles.labelRow}><Text style={styles.label}>{field.label}{field.required ? <Text style={{ color: colors.accent }}> *</Text> : ''} </Text>{!field.required && <Text style={styles.optional}>Optional</Text>}</View>
    {field.type === 'select' ? <>
    ...
    </> : <TextInput selectionColor={colors.primary} accessibilityLabel={field.label} value={String(value ?? '')} editable={!disabled} onChangeText={onChange} placeholder={field.type === 'number' ? '0.00' : field.label} placeholderTextColor={colors.subtle} keyboardType={field.type === 'number' ? 'decimal-pad' : field.type === 'email' ? 'email-address' : 'default'} autoCapitalize={field.type === 'email' ? 'none' : 'sentences'} multiline={field.type === 'multiline'} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={[styles.input, focused && { borderColor: colors.primary, backgroundColor: colors.surface }, disabled && { opacity: .6 }, field.type === 'multiline' && { minHeight: 100, textAlignVertical: 'top' }]} />}
  </View>
}
const styles = themedStyles(() => ({ labelRow: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }, optional: { color: colors.muted, fontSize: 12 }, field: { gap: 10 }, label: { color: colors.ink, fontSize: 14, lineHeight: 20, flexShrink: 1, fontWeight: '600' }, input: { borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised, borderRadius: 16, padding: 15, minHeight: 52, color: colors.ink, fontSize: 16 }, inputText: { color: colors.ink, fontSize: 16, flexShrink: 1 }, select: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, borderWidth: 1, borderColor: colors.border, borderRadius: 16, padding: 16, minHeight: 52, backgroundColor: colors.surface }, toggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 52 }, option: { paddingVertical: 18, flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border } }))
