  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={95}>
    <View style={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: colors.border, backgroundColor: colors.surface }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        ...
        <View style={{ flex: 1 }}><Text style={[s.sectionTitle, { fontSize: 20 }]}>Vyom</Text><Text style={[s.caption, { marginTop: 0 }]} numberOfLines={2} ellipsizeMode="tail">{activeConversation?.title || 'New conversation'}</Text></View>
    ...
    <ScrollView ref={chatScroll} style={{ flex: 1 }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: 28, gap: 14 }} onContentSizeChange={() => { if (messages.length) chatScroll.current?.scrollToEnd({ animated: true }) }}>
      ...
      {!messages.length && <View style={{ alignItems: 'center', paddingVertical: 28, gap: 10, paddingHorizontal: 20 }}><View style={{ padding: 16, borderRadius: 22, backgroundColor: colors.softBlue }}><Icon name="sparkles-outline" size={30} color={colors.accent} /></View><Text style={s.title}>Ask Vyom</Text><Text style={[s.caption, { textAlign: 'center', maxWidth: 280 }]}>Ask about your workspace, its features, or a general topic.</Text></View>}
