...
export function AskAIScreen() {
  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={95}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.content}>
    <View style={{ alignItems: 'center', gap: 12, paddingVertical: 12 }}><View style={{ padding: 20, borderRadius: 25, backgroundColor: colors.softBlue }}><Icon name="sparkles-outline" size={36} color={colors.accent} /></View><Text style={s.title}>Ask your workspace</Text><Text style={[s.caption, { textAlign: 'center' }]}>Answers from your financial records, with sources.</Text></View>
...
export function FlowScreen() {
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={checks.loading} onRefresh={checks.refresh} />}>
    <Text style={s.title}>Workspace intelligence</Text><Text style={s.caption}>Review signals, plan ahead, and ask about your records.</Text><Button icon="sparkles-outline" label="Ask your workspace" onPress={() => nav.navigate('AskAI')} />
