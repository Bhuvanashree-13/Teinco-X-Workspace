...
export function Search({ value, onChangeText, placeholder = 'Search records' }: { value: string; onChangeText: (v: string) => void; placeholder?: string }) {
  useMobileTheme()
 return <View style={s.search}><Icon name="search-outline" color={colors.muted} size={20} /><TextInput selectionColor={colors.primary} accessibilityLabel={placeholder} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={colors.muted} style={s.searchInput} autoCapitalize="none" />{!!value && <Pressable accessibilityLabel="Clear search" hitSlop={10} onPress={() => onChangeText('')}><Icon name="close-circle" color={colors.muted} size={20} /></Pressable>}</View> }
...
export function Tag({ value }: { value?: string }) {
  useMobileTheme()
 if (!value) return null; const positive = ['active', 'approved', 'paid', 'present', 'complete', 'completed', 'resolved', 'received'].includes(value); return <View style={[s.tag, positive && { backgroundColor: colors.softGreen }]}><Text style={[s.tagText, positive && { color: colors.success }]}>{value.replace(/_/g, ' ')}</Text></View> }
...
export function LoadState({ loading, error, retry }: { loading: boolean; error?: string | null; retry?: () => void }) {
  useMobileTheme()
 if (loading) return <View style={s.empty}><ActivityIndicator color={colors.accent} /><Text style={s.caption}>Loading your records…</Text></View>; if (!error) return null; return <View style={s.error}><Text accessibilityRole="alert" style={s.errorText}>{error}</Text>{retry && <Button secondary label="Try again" onPress={retry} />}</View> }
export function Section({ title, action, onPress }: { title: string; action?: string; onPress?: () => void }) {
  useMobileTheme()
 return <View style={s.section}><Text style={s.sectionTitle}>{title}</Text>{action && <Pressable onPress={onPress} hitSlop={8}><Text style={s.link}>{action}</Text></Pressable>}</View> }
...
export const s = themedStyles(() => ({
  page: { flex: 1, backgroundColor: colors.background }, content: { padding: 20, paddingBottom: 42, gap: 18 }, title: { color: colors.ink, fontSize: 29, fontWeight: '700', letterSpacing: -.9 }, caption: { color: colors.muted, fontSize: 13, lineHeight: 20, marginTop: 4 },
  panel: { backgroundColor: colors.surface, borderRadius: 24, padding: 19, borderWidth: 1, borderColor: colors.border, gap: 11, shadowColor: colors.shadow, shadowOpacity: .07, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 2 },
  button: { minHeight: 52, borderRadius: 17, paddingHorizontal: 18, paddingVertical: 14, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, shadowColor: colors.shadow, shadowOpacity: .18, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 2 }, secondary: { backgroundColor: colors.softBlue, shadowOpacity: 0 }, buttonText: { fontSize: 14, fontWeight: '700' },
  ...
  tag: { borderRadius: 8, backgroundColor: colors.softAmber, paddingHorizontal: 8, paddingVertical: 4, alignSelf: 'flex-start' }, tagText: { fontSize: 10, fontWeight: '700', color: colors.amber, textTransform: 'capitalize' },
  ...
  section: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }, sectionTitle: { fontWeight: '700', fontSize: 18, color: colors.ink, letterSpacing: -.3 }, link: { color: colors.accent, fontSize: 13, fontWeight: '700' },
