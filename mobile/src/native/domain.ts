...
export type Field = { key: string; label: string; type?: 'text' | 'number' | 'date' | 'datetime' | 'select' | 'boolean' | 'email' | 'multiline' | 'image'; required?: boolean; min?: number; max?: number; integer?: boolean; default?: string | number | boolean; choices?: string[]; lookup?: string; admin?: boolean; section?: string; createOnly?: boolean }
...
export const modules: Module[] = [
  ...
  { id: 'employees', title: 'Directory', singular: 'Employee', endpoint: '/employees', icon: 'people-outline', group: 'people', create: true, edit: true, deleteLabel: 'Archive employee', fields: [text('name', 'Full name', true), f('email', 'Work email', { type: 'email', required: true }), text('phone', 'Phone'), text('role', 'Job title'), text('department', 'Department'), text('managerName', 'Manager'), text('workLocation', 'Work location'), select('employmentType', 'Employment type', ['full_time', 'part_time', 'contractor', 'freelancer', 'intern']), { ...number('monthlyCost', 'Monthly compensation'), admin: true }, date('startDate', 'Start date', false), select('status', 'Status', ['active', 'on_leave', 'terminated', 'contractor']), notes] },
  ...
  { id: 'attendance', title: 'Attendance', singular: 'Attendance entry', endpoint: '/employees/attendance', icon: 'finger-print-outline', group: 'people', fields: [employee, date('workDate', 'Work date'), text('workMode', 'Work mode'), text('status', 'Status')] },
  ...
  { id: 'projects', title: 'Projects', singular: 'Project', endpoint: '/projects', icon: 'folder-open-outline', group: 'people', create: true, edit: true, detail: true, deleteLabel: 'Delete project', fields: [f('logoDataUrl', 'Project logo', { type: 'image' }), text('name', 'Project name', true), text('code', 'Project key'), select('status', 'Status', ['active', 'paused', 'completed', 'archived']), number('budget', 'Budget'), f('description', 'Description', { type: 'multiline' })] },
  { id: 'taskboard', title: 'Projects & Taskboard', singular: 'Work item', endpoint: '/projects/issues', icon: 'checkbox-outline', group: 'people', create: true, edit: true, detail: true, deleteLabel: 'Delete work item', fields: [lookup('projectId', 'Project', '/projects', true), text('summary', 'Summary', true), select('type', 'Type', ['epic', 'story', 'task', 'bug']), select('status', 'Workflow', ['backlog', 'todo', 'in_progress', 'review', 'blocked', 'done']), select('priority', 'Priority', ['lowest', 'low', 'medium', 'high', 'highest']), { ...lookup('assigneeId', 'Assignee', '/employees'), required: false }, text('sprint', 'Sprint'), { ...number('storyPoints', 'Story points'), required: false, integer: true, max: 100 }, date('dueDate', 'Due date', false), text('labels', 'Labels'), f('description', 'Description', { type: 'multiline' })] },
  ...
  { id: 'automation', title: 'Automation', singular: 'Automation rule', endpoint: '/flow/automation-rules', icon: 'flash-outline', group: 'flow', admin: true, create: true, hint: 'Active email automations run every 15 minutes. Delivery is recorded so each payslip or due-date notice is sent once.', fields: [text('name', 'Rule name', true), select('triggerModule', 'Trigger module', ['ledger', 'people', 'schedule', 'flow']), text('triggerEvent', 'Trigger event', true), select('actionModule', 'Action module', ['schedule', 'people', 'ledger', 'flow', 'automation']), f('actionSummary', 'Action', { type: 'multiline', required: true }), priority, notes] },
...
export const canWrite = (module: Module, role: Role, editing = false) => module.id !== 'attendance' && (role === 'admin' || Boolean(module.employeeWrite) || (!editing && ['vendors', 'subscriptions', 'taskboard'].includes(module.id)))
...
export function recordTitle(id: string, row: Row) {
  ...
  return row.description && id === 'expenses' ? row.description : row.summary || row.source || row.productName || row.name || row.title || row.batchId || 'Record'
}
export function recordSubtitle(id: string, row: Row) {
  ...
  if (id === 'attendance') return `${human(row.workMode)} · ${row.checkedOut ? 'Checked out' : row.checkedIn ? 'Checked in' : human(row.status)}`
  if (id === 'balances') return `${row.balance} days available · ${row.used} used`
  if (id === 'taskboard') return [row.key, row.project?.name, row.assignee?.name].filter(Boolean).join(' · ')
