...
export const modules: Module[] = [
  ...
  { id: 'lifecycle', title: 'HR tasks', singular: 'HR task', endpoint: '/employees/lifecycle', icon: 'checkbox-outline', group: 'people', admin: true, create: true, fields: [text('title', 'Task', true), { ...employee, required: false }, select('taskType', 'Task type', ['onboarding', 'offboarding']), f('ownerTeam', 'Owner team', { default: 'HR' }), date('dueDate', 'Due date', false), f('checklist', 'Checklist', { type: 'multiline' })] },
  { id: 'events', title: 'Events', singular: 'Event', endpoint: '/schedule/events', icon: 'calendar-outline', group: 'schedule', create: true, employeeWrite: true, fields: [text('title', 'Event title', true), date('startsAt', 'Starts', true, true), date('endsAt', 'Ends', true, true), select('eventType', 'Event type', ['meeting', 'availability', 'holiday', 'deadline', 'audit', 'tax', 'launch', 'personal']), priority, text('location', 'Location'), text('ownerName', 'Organizer'), lookup('projectId', 'Project', '/projects'), f('description', 'Description', { type: 'multiline' }), f('isAllDay', 'All-day event', { type: 'boolean' })] },
...
export function recordDate(row: Row): string | undefined { return row.expenseDate || row.depositDate || row.nextBillingDate || row.workDate || row.startsAt || row.dueAt || row.dueDate || row.periodStart || row.startDate }
