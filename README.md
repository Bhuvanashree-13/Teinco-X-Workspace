...
Local development reads `.env` when running `npm run dev`. Set `VYOM_OLLAMA_URL` to your Ollama base URL (for example `http://127.0.0.1:11434`) and `ASK_AI_MODEL` to an installed model from `ollama list`. Model requests time out after 180 seconds by default; set `VYOM_TIMEOUT_MS` between 10,000 and 600,000 to override it. Optionally set `ASK_AI_API_KEY` for an authenticated Ollama gateway. Restart the app server after changing these values.
